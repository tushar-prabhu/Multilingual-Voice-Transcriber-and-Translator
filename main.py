    ...
    def run(self):
        try:
            ...
            try:
                ...
                self.status_signal.emit(
                    "Could not request Google Speech Recognition service; "
                    + str(e)
                )
...
class VoiceConverterApp(QWidget):
    ...
    def init_ui(self):
        ...
        # import language codes from a JSON file
        ...
        # Set a default value for to_lang
    ...
    def start_recording(self):
        if not self.recording:
            ...
            self.selected_language = self.language_dict.get(
                self.language_dropdown.currentText(),
                ""
            )
            if self.selected_language:
                # Set the target language
                self.translator.to_lang = self.selected_language
    ...
    def download_audio(self):
        translated_text = self.translated_label.toPlainText().replace(
            'Translated Text: ',
            ''
        )
        tts = gTTS(translated_text, lang=self.selected_language)
